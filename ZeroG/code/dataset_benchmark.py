...
NODEDATASET = [
    'cora', 'citeseer', 'wikics', 'photo', 'computer', 'instagram',
]
...
def load_node_dataset(root, name, args):
    ...
    if name in ['yelp', 'amazon']:
    ...
    elif name in ['cora', 'citeseer', 'wikics', "photo", "computer", 'instagram']:

        data = torch.load(f"../../../datasets/test/{name.lower()}.pt")
        ...
        if name.lower() == 'cora':
            data.label_name = ['Case Based', 'Genetic Algorithms', 'Neural Networks', 'Probabilistic Methods', 'Reinforcement Learning', 'Rule Learning', 'Theory']
        elif name.lower() == 'citeseer':
            data.label_name = ['Agents', 'Machine Learning', 'Information Retrieval', 'Database', 'Human Computer Interaction', 'Artificial Intelligence']
        elif name.lower() == 'wikics':
            data.label_name = ['Computational linguistics', 
                    'Databases', 
                    'Operating systems', 
            ...
            data.label_name = ['Normal Users', 'Commercial Users']
        elif name.lower() == 'history':
            data.label_name = ['World', 'Americas', 'Asia', 'Military', 'Europe', 'Russia', 'Africa', 'Ancient Civilizations', 'Middle East', 'Historical Study & Educational Resources', 'Australia & Oceania', 'Arctic & Antarctica']
        elif name.lower() == 'photo':
            data.label_name = ['Video Surveillance', 'Accessories', 'Binoculars & Scopes', 'Video', 'Lighting & Studio', 'Bags & Cases', 'Tripods & Monopods', 'Flashes', 'Digital Cameras', 'Film Photography', 'Lenses', 'Underwater Photography']
        elif name.lower() == 'computer':
            data.label_name = ['Computer Accessories & Peripherals', 'Tablet Accessories', 'Laptop Accessories', 'Computers & Tablets', 'Computer Components', 'Data Storage', 'Networking Products', 'Monitors', 'Servers', 'Tablet Replacement Parts']



        data.label_text = data.label_name

        # data1 = torch.load(f"../datasets/{name.lower()}_clip.pt")
        # with open("reddit.txt", "w", encoding="utf-8") as f:
        #     for line in data1.raw_texts:
        #         f.write(line.strip() + "\n")
