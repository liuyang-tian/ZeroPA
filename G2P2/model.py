...
class GNN(MessagePassing):
    ...
    @staticmethod
    def norm(edge_index, edge_weight, num_nodes, improved=False, dtype=None):
    ...
    def forward(self, x, edge_index, edge_weight, vars=None):
        if vars is None:
            vars = self.vars
        improved = False

        w, b = vars[0], vars[1]
        edge_index, norm = self.norm(edge_index, edge_weight, x.size(self.node_dim), improved, x.dtype)
...
class CLIP(nn.Module):
    ...
    def forward(self, x, adj, s_n, s_n_text, t_n_text, device):
