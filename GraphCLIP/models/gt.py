...
from torch_geometric.nn import GPSConv, global_mean_pool, SAGPooling
from torch_geometric.nn import SimpleConv
...
class GPS(torch.nn.Module):
    ...
    def forward(self, x, pe, edge_index, edge_weight, batch, center_idx):
        x_pe = self.pe_norm(pe)
        x = torch.cat((self.node_emb(x.squeeze(-1)), self.pe_lin(x_pe)), 1)
        for conv in self.convs:
            x = conv(x, edge_index, batch=batch, edge_weight=edge_weight)
